...
export class SettingsService {
  ...
  async updateGlobalSettings(updates: Partial<GlobalSettings>): Promise<GlobalSettings> {
    ...
    // Deep merge provider settings if provided
    if (updates.providerSettings) {
      updated.providerSettings = {
        ...current.providerSettings,
        ...updates.providerSettings,
        openai: {
          ...current.providerSettings?.openai,
          ...updates.providerSettings.openai,
        },
        openrouter: {
          ...current.providerSettings?.openrouter,
          ...updates.providerSettings.openrouter,
        },
      };
    }
  ...
  async getMaskedCredentials(): Promise<{
    ...
    return {
      ...
      openrouter: {
        configured: !!credentials.apiKeys.openrouter,
        masked: maskKey(credentials.apiKeys.openrouter),
      },
    };
  }
