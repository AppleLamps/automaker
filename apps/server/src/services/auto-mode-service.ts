...
import { getFeatureDir, getAutomakerDir, getFeaturesDir, isPathAllowed } from '@automaker/platform';
...
const EXECUTION_PHASE_ORDER: ExecutionPhase[] = [
  'initialized',
  'planned',
  'executing',
  'verifying',
...
const MAX_AUTO_LOOP_ITERATIONS = 10000;
const MAX_AUTO_LOOP_IDLE_CYCLES = 60;
...
export class AutoModeService {
  ...
  private executionPhases = new Map<string, ExecutionPhase>();
  private hotSpotPatternsCache = new Map<string, string[]>();
  ...
  private async runAutoLoop(): Promise<void> {
    ...
    while (
      ...
      try {
        iterationCount += 1;
        if (iterationCount > MAX_AUTO_LOOP_ITERATIONS) {
          this.emitAutoModeEvent('auto_mode_failed', {
            reason: 'Auto mode loop exceeded iteration cap',
            location: 'AutoModeService.runAutoLoop',
            phase: 'executing',
            retryable: false,
        ...
        if (pendingFeatures.length === 0) {
          ...
          if (this.runningFeatures.size === 0) {
            idleCycles += 1;
            if (idleCycles >= MAX_AUTO_LOOP_IDLE_CYCLES) {
              this.emitAutoModeEvent('auto_mode_failed', {
                reason: 'Auto mode idle cap reached without pending features',
                location: 'AutoModeService.runAutoLoop',
                phase: 'executing',
                retryable: true,
                projectPath: this.config!.projectPath,
  ...
  async executeFeature(
    ...
    try {
      ...
      this.executionPhases.delete(featureId);
      await this.setExecutionPhase(
        projectPath,
        featureId,
        'initialized',
      ...
      if (errorInfo.isAbort) {
        ...
        const failureInfo: FailureInfo = {
          reason: 'Feature stopped by user',
          location: 'AutoModeService.executeFeature',
          phase: 'failed',
          featureId,
          projectPath,
          retryable: false,
        };
        await this.updateFeatureFailure(projectPath, featureId, failureInfo);
        await this.setExecutionPhase(
          projectPath,
          featureId,
          'failed',
        ...
        this.emitAutoModeEvent('auto_mode_failed', failureInfo);
      } else {
...
Format your response as a structured markdown document.`;
  ...
  private async updateFeatureExecutionPhase(
    projectPath: string,
    featureId: string,
    phase: ExecutionPhase
  ): Promise<void> {
    ...
    try {
      const data = (await secureFs.readFile(featurePath, 'utf-8')) as string;
      const feature = JSON.parse(data);
      feature.executionPhase = phase;
      feature.updatedAt = new Date().toISOString();
      await secureFs.writeFile(featurePath, JSON.stringify(feature, null, 2));
    } catch (error) {
      console.error(`[AutoMode] Failed to update executionPhase for ${featureId}:`, error);
    }
  }

  private async updateFeatureFailure(
    projectPath: string,
    featureId: string,
    failure: FailureInfo
  ): Promise<void> {
    ...
    try {
      const data = (await secureFs.readFile(featurePath, 'utf-8')) as string;
      const feature = JSON.parse(data);
      feature.error = failure.reason;
      feature.errorDetails = {
      ...
      console.error(`[AutoMode] Failed to update failure details for ${featureId}:`, error);
    }
  }

  private async setExecutionPhase(
    projectPath: string,
    featureId: string,
    nextPhase: ExecutionPhase,
    location: string
  ): Promise<void> {
    const current = this.executionPhases.get(featureId);
    if (current === nextPhase) {
      return;
    }
    if (current) {
      const currentIndex = EXECUTION_PHASE_ORDER.indexOf(current);
      const nextIndex = EXECUTION_PHASE_ORDER.indexOf(nextPhase);
      if (nextIndex < currentIndex) {
        throw new Error(
          `Execution phase regression for ${featureId}: ${current} -> ${nextPhase} at ${location}`
        );
      }
    }

    this.executionPhases.set(featureId, nextPhase);
    await this.updateFeatureExecutionPhase(projectPath, featureId, nextPhase);
    this.emitAutoModeEvent('auto_mode_phase_changed', {
      featureId,
      projectPath,
      phase: nextPhase,
    });
  ...
  private async loadHotSpotPatterns(projectPath: string): Promise<string[]> {
    if (this.hotSpotPatternsCache.has(projectPath)) {
      return this.hotSpotPatternsCache.get(projectPath)!;
    }
    ...
    try {
      ...
      const patterns = data
        ...
        .filter(Boolean);
      const finalPatterns = patterns.length > 0 ? patterns : DEFAULT_HOT_SPOT_PATTERNS;
      this.hotSpotPatternsCache.set(projectPath, finalPatterns);
      return finalPatterns;
    } catch {
      this.hotSpotPatternsCache.set(projectPath, DEFAULT_HOT_SPOT_PATTERNS);
...
`;
    ...
    await this.setExecutionPhase(projectPath, featureId, 'planned', 'AutoModeService.runAgent');

    // Execute via provider
    const stream = provider.executeQuery(executeOptions);
    await this.setExecutionPhase(projectPath, featureId, 'executing', 'AutoModeService.runAgent');
    ...
    const hotSpotPatterns = await this.loadHotSpotPatterns(finalProjectPath);
    ...
    const normalizeScope = (files: string[]): string[] => {
      const resolved = files.map((file) => path.resolve(workDir, file));
    ...
    const validateAndSetScope = (intent: { summary: string; files: string[] }) => {
      ...
      const hotSpots = resolved.filter((filePath) =>
        this.isHotSpotPath(filePath, hotSpotPatterns, finalProjectPath)
      );
      if (hotSpots.length > 0 && options?.requirePlanApproval !== true) {
        throw new Error(
          `Hot-spot edits require plan approval: ${hotSpots
            .map((filePath) => path.relative(finalProjectPath, filePath))
            .join(', ')}`
        );
      }
                ...
                while (!planApproved) {
                  ...
                  try {
                    ...
                    if (approvalResult.approved) {
                      ...
                      revisionAttempts += 1;
                      if (revisionAttempts > MAX_PLAN_REVISIONS) {
                        await this.updateFeaturePlanSpec(projectPath, featureId, {
                          status: 'rejected',
                        });
                        throw new Error('Plan revision limit exceeded');
                      }
                      console.log(
                      ...
                      const previousPlanContent = currentPlanContent;
...
`;
                      ...
                      if (currentPlanContent.trim() === previousPlanContent.trim()) {
                        unchangedPlanCount += 1;
                        if (unchangedPlanCount >= 2) {
                          await this.updateFeaturePlanSpec(projectPath, featureId, {
                            status: 'rejected',
                          });
                          throw new Error('Plan revision did not change after feedback');
                        }
                      } else {
                        unchangedPlanCount = 0;
                      }
