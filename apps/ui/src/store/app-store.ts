...
const shouldPersistApiKeys = (): boolean => {
...
async function validateProjectSelection(project: Project): Promise<boolean> {
  const result = await validateProjectDirectory(project.path);
  if (result.status === 'ok') {
    return true;
  }
    ...
    (set, get) => ({
      ...
      restoreTrashedProject: (projectId) => {
        ...
        if (samePathProject) {
          ...
          void get().setCurrentProject(samePathProject);
          return;
        }
        ...
        void get().setCurrentProject(restoredProject);
      },
      ...
      setCurrentProject: (project) => {
        if (!project) {
          set({ currentProject: null, currentView: 'welcome' });
          return;
        }

        void (async () => {
          const isValid = await validateProjectSelection(project);
          if (!isValid) {
            return;
          }

          set({ currentProject: project, currentView: 'board' });
      ...
      upsertAndSetCurrentProject: (path, name, theme) => {
        ...
        void get().setCurrentProject(project);
        return project;
      },
      ...
      cyclePrevProject: () => {
        void (async () => {
          ...
          const isValid = await validateProjectSelection(targetProject);
          if (!isValid) {
            return;
          }
    ...
    }),
    {
      ...
      merge: (persistedState, currentState) => {
        ...
        if (!shouldPersistApiKeys()) {
          merged.apiKeys = current.apiKeys;
        }
      ...
      partialize: (state) =>
        ({
          ...
          // Settings
          apiKeys: shouldPersistApiKeys() ? state.apiKeys : undefined,
