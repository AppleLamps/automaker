...
export function useSettingsMigration(): MigrationState {
  ...
  useEffect(() => {
    ...
    const requestIds = {
      status: 'settingsMigration:status',
      migrate: 'settingsMigration:migrate',
    };
    let cancelled = false;

    const safeSetState = (next: MigrationState) => {
      if (!cancelled) {
        setState(next);
      }
    };
    ...
    const cleanup = () => {
      cancelled = true;
      api.cancelRequest(requestIds.status);
      api.cancelRequest(requestIds.migrate);
    };

    async function checkAndMigrate() {
      // Only run migration in Electron mode (web mode uses different storage)
      if (!isElectron()) {
        safeSetState({ checked: true, migrated: false, error: null });
        return;
      }
      ...
      try {
        ...
        if (result.success) {
          ...
          safeSetState({ checked: true, migrated: true, error: null });
        } else {
        ...
        safeSetState({
          checked: true,
          migrated: false,
          error: formatUserError(error, 'Unknown error'),
        });
      }
    }
 ...
 * Only functions in Electron mode. Returns false if not in Electron or on error.
 *
 * @param apiKeys - Partial credential object with optional anthropic, google, openai, openrouter keys
 * @returns Promise resolving to true if sync succeeded, false otherwise
 */
