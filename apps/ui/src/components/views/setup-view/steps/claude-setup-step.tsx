...
export function ClaudeSetupStep({ onNext, onBack, onSkip }: ClaudeSetupStepProps) {
  ...
  // Saved states for visual feedback
  ...
  const { isSaving: isSavingOpenRouterKey, saveToken: saveOpenRouterToken } = useTokenSave({
    provider: 'openrouter',
    onSuccess: () => {
      setApiKeys({ ...apiKeys, openrouter: openRouterKey });
      setOpenRouterSaved(true);
      setTimeout(() => setOpenRouterSaved(false), 2000);
    },
  });

  const { isSaving: isSavingOpenAiKey, saveToken: saveOpenAiToken } = useTokenSave({
    provider: 'openai',
    onSuccess: () => {
      setApiKeys({ ...apiKeys, openai: openAiKey });
      setOpenAiSaved(true);
  ...
  const deleteOpenRouterKey = useCallback(async () => {
    try {
      const api = getElectronAPI();
      if (!api.setup?.deleteApiKey) {
        toast.error('Delete API not available');
  ...
  // User is ready if any auth method is configured
  const hasApiKey =
    !!apiKeys.anthropic ||
  ...
  // Ready if CLI verified, Anthropic key verified, or alternative provider key saved
  const isReady = isCliVerified || isApiKeyVerified || hasOpenRouterKey || hasOpenAiKey;
  ...
  // Helper to get status badge for OpenRouter
  const getOpenRouterStatusBadge = () => {
    if (hasOpenRouterKey) {
      return <StatusBadge status="authenticated" label="Configured" />;
    }
    return <StatusBadge status="not_authenticated" label="Not Set" />;
  };

  // Helper to get status badge for OpenAI
      ...
      <Card className="bg-card border-border">
        ...
        <CardContent>
          <Accordion type="single" collapsible className="w-full">
            ...
            {/* Option 3: OpenRouter API Key */}
            <AccordionItem value="openrouter" className="border-border">
              <AccordionTrigger className="hover:no-underline">
                <div className="flex items-center justify-between w-full pr-4">
                  <div className="flex items-center gap-3">
                    <Globe
                      className={`w-5 h-5 ${
                        hasOpenRouterKey ? 'text-green-500' : 'text-muted-foreground'
                      }`}
                    />
                    <div className="text-left">
              ...
              <AccordionContent className="pt-4 space-y-4">
                <div className="space-y-4 p-4 rounded-lg bg-muted/30 border border-border">
                  ...
                  <div className="flex gap-2">
                    <Button
                      onClick={() => saveOpenRouterToken(openRouterKey)}
                      disabled={isSavingOpenRouterKey || !openRouterKey.trim() || openRouterSaved}
                      className="flex-1 bg-brand-500 hover:bg-brand-600 text-white"
                      data-testid="save-openrouter-key-button"
                    >
                    ...
                    {hasOpenRouterKey && (
                      <Button
                        ...
                        variant="outline"
                        className="border-red-500/50 text-red-500 hover:bg-red-500/10 hover:text-red-400"
                        data-testid="delete-openrouter-key-button"
                      >
                        <Trash2 className="w-4 h-4" />
                      </Button>
                    )}
                ...
                {hasOpenRouterKey && (
                  <div className="flex items-center gap-3 p-4 rounded-lg bg-green-500/10 border border-green-500/20">
                    <CheckCircle2 className="w-5 h-5 text-green-500" />
                    <div>
                      <p className="font-medium text-foreground">OpenRouter API Key configured!</p>
                      <p className="text-sm text-muted-foreground">
                        You can use OpenRouter models in your projects.
                      </p>
                    </div>
                  </div>
                )}
            ...
            <AccordionItem value="openai" className="border-border">
              <AccordionTrigger className="hover:no-underline">
                <div className="flex items-center justify-between w-full pr-4">
                  <div className="flex items-center gap-3">
                    <Sparkles
                      className={`w-5 h-5 ${
                        hasOpenAiKey ? 'text-green-500' : 'text-muted-foreground'
                      }`}
                    />
                    <div className="text-left">
                      <p className="font-medium text-foreground">OpenAI API Key</p>
                      <p className="text-sm text-muted-foreground">
                        Use GPT models with your OpenAI key
                      </p>
                    </div>
                  </div>
              ...
              <AccordionContent className="pt-4 space-y-4">
                <div className="space-y-4 p-4 rounded-lg bg-muted/30 border border-border">
                  ...
                  <div className="flex gap-2">
                    <Button
                      onClick={() => saveOpenAiToken(openAiKey)}
                      disabled={isSavingOpenAiKey || !openAiKey.trim() || openAiSaved}
                      className="flex-1 bg-brand-500 hover:bg-brand-600 text-white"
                    ...
                    {hasOpenAiKey && (
                      <Button
                        ...
                        variant="outline"
                        className="border-red-500/50 text-red-500 hover:bg-red-500/10 hover:text-red-400"
                        data-testid="delete-openai-key-button"
                      >
                        <Trash2 className="w-4 h-4" />
                      </Button>
                    )}
                ...
                {hasOpenAiKey && (
                  <div className="flex items-center gap-3 p-4 rounded-lg bg-green-500/10 border border-green-500/20">
                    <CheckCircle2 className="w-5 h-5 text-green-500" />
                    <div>
                      <p className="font-medium text-foreground">OpenAI API Key configured!</p>
                      <p className="text-sm text-muted-foreground">
                        You can use OpenAI models in your projects.
                      </p>
                    </div>
                  </div>
                )}
