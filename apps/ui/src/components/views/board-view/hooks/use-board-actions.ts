...
export function useBoardActions({
  ...
  const handleRetryFeature = useCallback(
    async (feature: Feature) => {
      ...
      // Clear error and reset feature to in_progress
      const updates = {
        status: 'in_progress' as const,
        error: undefined,
      ...
      toast.info('Retrying feature', {
        description: `Restarting: ${truncateDescription(feature.description)}`,
      });

      // Run the feature again
      await handleRunFeature(feature);
    },
    [currentProject, autoMode, updateFeature, persistFeatureUpdate, handleRunFeature]
  );
