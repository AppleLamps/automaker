...
export function useBoardFeatures({ currentProject }: UseBoardFeaturesProps) {
  ...
  const getLegacyFeatureId = useCallback((feature: Feature, index: number) => {
    const existingLegacyId =
      typeof (feature as Feature & { legacyId?: unknown }).legacyId === 'string'
        ? (feature as Feature & { legacyId?: string }).legacyId
        : null;
    if (existingLegacyId) {
    ...
    const rawKey = [
      ...
      .filter(Boolean)
      .join('|');
    const key = rawKey || `feature-${index}`;
    let hash = 5381;
  ...
  const missingProjectToastRef = useRef<string | null>(null);
    ...
    try {
      ...
      if (result.success && result.features) {
        const featuresWithIds = result.features.map((f: any, index: number) => {
          const legacyId = !f.id ? getLegacyFeatureId(f, index) : undefined;
          return {
            ...f,
            ...(legacyId ? { legacyId } : {}),
            id: f.id || legacyId,
        ...
        const isMissingProject =
          result.error.includes('Project directory not found') ||
          result.error.includes('Project path is not a directory');
        if (isMissingProject) {
          if (missingProjectToastRef.current !== currentPath) {
            missingProjectToastRef.current = currentPath;
            toast.error('Project folder missing', {
  ...
  const saveCategory = useCallback(
    async (category: string) => {
      ...
      try {
        const api = getElectronAPI();
        const validation = await validateProjectDirectory(currentProject.path);
        if (validation.status !== 'ok') {
          toast.error('Project folder unavailable', {
        ...
        if (!categories.includes(category)) {
          ...
          if (!writeResult.success) {
            toast.error('Failed to save category', {
              description:
                writeResult.error || `Unable to write categories for ${currentProject.path}.`,
            });
            return;
          }
