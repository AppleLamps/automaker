...
export const KanbanColumn = memo(function KanbanColumn({
  ...
  const countId = `kanban-column-${id}-count`;
  ...
  return (
    ...
    >
      ...
      >
        ...
        <h3 id={titleId} className="font-semibold text-sm text-foreground/90 flex-1 tracking-tight">
          {title}
        </h3>
        {headerAction}
        <span
          id={countId}
          className="text-xs font-medium text-muted-foreground/80 bg-muted/50 px-2 py-0.5 rounded-md tabular-nums"
        >
          {count}
        </span>
      </div>
