  ...
  return (
    <div className="flex flex-wrap gap-1.5 -mx-3 -mb-3 px-3 pb-3">
      ...
      {!isCurrentAutoTask && feature.status === 'waiting_approval' && (
        <>
          {/* Retry button - only show when feature has an error */}
          {feature.error && onRetry && (
            <Button
              variant="default"
              size="sm"
              className="flex-1 h-7 text-[11px] min-w-0 bg-[var(--status-warning)] hover:bg-[var(--status-warning)]/90"
              onClick={(e) => {
                e.stopPropagation();
                onRetry();
              }}
              onPointerDown={(e) => e.stopPropagation()}
              data-testid={`retry-${feature.id}`}
            >
