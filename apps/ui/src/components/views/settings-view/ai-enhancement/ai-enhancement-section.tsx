...
export function AIEnhancementSection() {
  const { enhancementModel, setEnhancementModel, availableModels } =
    useAppStore();
  const modelOptions = useMemo(
    () =>
      getModelOptions(availableModels).filter(
      ...
      <div className="p-6 space-y-4">
        <div className="space-y-4">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
            {modelOptions.map(({ id, label, description, badge }) => {
              ...
              return (
                <button
                  ...
                  className={cn(
                    ...
                    isActive
                      ? [
                        'bg-linear-to-br from-brand-500/15 to-brand-600/10',
