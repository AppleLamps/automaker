...
export function ApiKeysSection() {
  const { apiKeys, setApiKeys, aiProfiles, defaultAIProfileId, setDefaultAIProfileId } = useAppStore();
  ...
  // Find the active AI profile
  const activeProfile = defaultAIProfileId
    ? aiProfiles.find((p) => p.id === defaultAIProfileId)
    : null;

  // Handle setting active provider by finding a profile that uses it
  const handleSetActiveProvider = useCallback((provider: ModelProvider) => {
    // Find the first profile that uses this provider
    const profileForProvider = aiProfiles.find((p) => p.provider === provider);
    if (profileForProvider) {
      setDefaultAIProfileId(profileForProvider.id);
      toast.success(`Switched to ${profileForProvider.name} profile`);
    } else {
      toast.error(`No AI profile found for ${provider}. Create one in AI Profiles settings.`);
    }
  }, [aiProfiles, setDefaultAIProfileId]);
    ...
    >
      ...
      <div className="p-6 space-y-6">
        ...
        <SecurityNotice
          message={
            isElectron
              ? 'API keys are stored locally on this device. Never share your API keys or commit them to version control.'
