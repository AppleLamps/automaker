...
function TerminalTabButton({
  ...
  const panelId = `terminal-tabpanel-${tab.id}`;
  ...
  useEffect(() => {
    ...
    const restoreLayout = async () => {
      ...
      if (restoringProjectPathRef.current !== currentPath) {
        logger.debug('Restore cancelled - project changed');
        return;
      }
      ...
      try {
        ...
        for (let tabIndex = 0; tabIndex < savedLayout.tabs.length; tabIndex++) {
          ...
          if (restoringProjectPathRef.current !== currentPath) {
            logger.debug('Restore cancelled mid-loop - project changed');
            return;
          }
        ...
        >
          <TerminalPanel
            ...
            onSessionInvalid={() => {
              ...
              logger.warn(`Session ${content.sessionId} is invalid, removing from layout`);
              killTerminal(content.sessionId);
            }}
    ...
    >
      <div className="flex-1 flex flex-col overflow-hidden">
        ...
        <div className="flex items-center bg-card border-b border-border px-2">
          {/* Tabs */}
          <div
            className="flex items-center gap-1 flex-1 overflow-x-auto py-1"
            role="tablist"
        ...
        <div
          ...
          role="tabpanel"
          id={
            terminalState.activeTabId ? `terminal-tabpanel-${terminalState.activeTabId}` : undefined
          }
          aria-labelledby={
            terminalState.activeTabId ? `terminal-tab-${terminalState.activeTabId}` : undefined
          }
          tabIndex={0}
        >
          {terminalState.maximizedSessionId ? (
            ...
            >
              <TerminalPanel
                ...
                onSessionInvalid={() => {
                  const sessionId = terminalState.maximizedSessionId!;
                  logger.warn(`Maximized session ${sessionId} is invalid, removing from layout`);
                  killTerminal(sessionId);
                }}
                isDragging={false}
                isDropTarget={false}
