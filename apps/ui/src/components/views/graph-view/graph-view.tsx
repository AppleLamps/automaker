...
export function GraphView({
  ...
  return (
    ...
    >
      <h2 id={graphLabelId} className="sr-only">
        Task dependency graph
      </h2>
      <p id={graphSummaryId} className="sr-only" aria-live="polite">
        {`Showing ${filteredFeatures.length} task${filteredFeatures.length === 1 ? '' : 's'} in the dependency graph.`}
      </p>
      <ul className="sr-only">
        {filteredFeatures.map((feature) => {
          const title = feature.title || feature.description || 'Untitled task';
          const statusRaw = feature.status || 'unknown';
          const status = typeof statusRaw === 'string' ? statusRaw.replace(/_/g, ' ') : 'unknown';
          const dependencies = (feature.dependencies as string[] | undefined)?.length ?? 0;
          return (
            <li key={feature.id}>
              {`${title}. Status: ${status}. Dependencies: ${dependencies}.`}
            </li>
          );
        })}
      </ul>
