...
export function modelSupportsThinking(_model?: AgentModel | string): boolean {
  ...
  const model = _model.toString().toLowerCase();
  if (['haiku', 'sonnet', 'opus'].includes(model)) return true;
...
export function getModelDisplayName(model: AgentModel | string): string {
  ...
  const normalized = model.toString();
  if (normalized.startsWith('openrouter/')) {
    const label = normalized.split('/').slice(1).join('/');
    return `OpenRouter ${label}`;
  }
  if (normalized.startsWith('gpt-')) {
    return normalized.replace(/^gpt-/, 'GPT-');
  }
  if (/^o\d/i.test(normalized)) {
    return normalized.toUpperCase();
  }
  if (normalized.includes('claude-')) {
    return normalized.replace(/^claude-/, 'Claude ');
  }
  return normalized;
}
