...
export type ProjectPathValidationResult =
  | { status: 'ok' }
  | { status: 'missing' | 'not_directory' | 'error'; error: string };
...
export async function validateProjectDirectory(
  ...
  try {
    const exists = await api.exists(projectPath);
    if (!exists) {
      return {
        status: 'missing',
        error: `Project directory not found: ${projectPath}. It may have been moved or deleted. Remove it from the project list or choose the new location.`,
      };
    }

    const stats = await api.stat(projectPath);
    if (!stats.success) {
      return {
        status: 'error',
        error:
          stats.error || `Unable to verify project directory: ${projectPath}. Please try again.`,
      };
    }

    if (stats.stats && !stats.stats.isDirectory) {
      return {
        status: 'not_directory',
        error: `Project path is not a directory: ${projectPath}. Update the project to point at a folder.`,
      };
    }
    ...
    return {
      ...
      error:
        error instanceof Error
          ? error.message
          : 'Unable to verify project directory. Please try again.',
    };
  }
}
