  ...
  describe('resolveModelString', () => {
    ...
    describe('with unknown model keys', () => {
      ...
      it('should use custom default for unknown model key', () => {
        const customDefault = 'claude-opus-4-20241113';
        const result = resolveModelString('unknown-model', customDefault);
    ...
    describe('with OpenAI/OpenRouter model strings', () => {
      it('should pass through OpenAI gpt-* model strings', () => {
        const model = 'gpt-4o';
        const result = resolveModelString(model);
        expect(result).toBe(model);
      });

      it('should pass through OpenAI reasoning model strings', () => {
        const model = 'o1-mini';
        const result = resolveModelString(model);
        expect(result).toBe(model);
      });

      it('should pass through OpenRouter model strings', () => {
